...
const running: Set<number> = new Set(); // guards against overlapping checks
const runningSince: Map<number, number> = new Map(); // timestamp when check started (for hung watchdog)
...
async function sendWebhookNotification(
  ...
  try {
    ...
    await axios.post(
      webhookUrl,
      isDiscord ? { content: msgText } : { text: msgText },
      { timeout: 10000 },
    );
...
async function sendEmailNotification(
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      text:
        `Status Alert for Monitor: ${monitor.name}\n\n` +
    ...
    console.log(
      `[EMAIL] Sent ${statusText} alert for monitor ${monitor.id} to ${settings.notificationEmail}`,
    );
  } catch (err: any) {
    console.error(
      `[EMAIL ERROR] monitor=${monitor.id}:`,
      err?.message || err,
    );
  }
}
  ...
  try {
    if (type === 'http') {
      ...
      const attempt = async (): Promise<{
        up: boolean; latency: number; message: string;
      }> => {
      ...
      try {
        ...
        if (isConnectionError) {
          console.log(
            `[RETRY] monitor=${type} ${url}: connection error "${err?.code}", retrying once`,
          );
          try {
    ...
    if (type === 'tcp') {
      ...
      return new Promise<{ up: boolean; latency: number; message: string }>(
        (resolve) => {
          let settled = false;
          const done = (up: boolean, msg: string) => {
            if (!settled) {
              settled = true;
    ...
    if (type === 'icmp') {
      ...
      try {
        const { stdout } = await execFileAsync(
          ...
          { timeout: (timeout + 2) * 1000, killSignal: 'SIGTERM' },
        );
...
async function runCheck(
  ...
  if (up) {
    ...
    if (newFailures >= FAILURE_THRESHOLD) {
      ...
      console.log(
        `[CONFIRM DOWN] monitor=${monitorId} failed ${newFailures}/${FAILURE_THRESHOLD}`,
      );
    } else {
      confirmedStatus = previousStatus ?? 'up';
      console.log(
        `[PENDING] monitor=${monitorId} failure ${newFailures}/${FAILURE_THRESHOLD}, keeping ${confirmedStatus}`,
      );
    }
  }

  console.log(
    `[HEARTBEAT] monitor=${monitorId} type=${type} raw=${up ? 'up' : 'down'} ` +
  ...
  if (isTransition) {
    ...
    if (monitor) {
      console.log(
        `[NOTIFY] monitor=${monitorId} state change: ${previousStatus ?? 'none'} → ${confirmedStatus}`,
      );
...
export async function scheduleAllMonitors(): Promise<void> {
 ...
 */
export async function scheduleMonitorWithInterval(
  monitorId: number,
...
function _registerInterval(
    ...
    const timerId = setTimeout(async () => {
      ...
      if (running.has(monitorId)) {
        ...
        const maxSafeDuration =
          (currentMonitor.timeout || 10) * 1000 * HUNG_CHECK_MULTIPLIER;
        ...
        if (hungDuration > maxSafeDuration) {
          ...
          console.error(
            `[WATCHDOG] monitor=${monitorId}: check hung for ${hungDuration}ms ` +
          ...
          console.log(
            `[SKIP] monitor=${monitorId}: previous check still running ` +
      ...
      try {
        const confirmedStatus = await runCheck(
          monitorId,
          currentMonitor.type,
          currentMonitor.url,
          currentMonitor.port,
          currentMonitor.timeout,
          currentMonitor.keyword,
          currentMonitor.expectedStatus,
        );
  ...
  console.log(
    `Monitor ${monitorId}: interval ${intervalSeconds}s registered` +
    (initialDelayMs ? ` (first check in ${(initialDelayMs / 1000).toFixed(1)}s)` : ''),
  );
}

export function cancelMonitorSchedule(monitorId: number): void {
  const existing = intervals.get(monitorId);
  if (existing) {
    clearTimeout(existing);
    intervals.delete(monitorId);
